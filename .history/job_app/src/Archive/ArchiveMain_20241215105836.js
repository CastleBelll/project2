...
import Archive from './Archive'; // Active.js 컴포넌트를 import
import "./ArchiveMain.css";

const ArchiveMain = () => {
  const canvasRef = useRef(null); // three.js 렌더링을 위한 canvas
  const [images, setImages] = useState([]); // 이미지들 상태
  const [selectedImageInfo, setSelectedImageInfo] = useState(null); // 클릭된 이미지 상태 추가
  ...
  useEffect(() => {
    ...
    const handleMouseClick = (event) => {
      ...
      if (intersects.length > 0) {
        ...
        setSelectedImageInfo(clickedPlane.userData.id);
      }
    };
  ...
  return (
    ...
    >
      ...
      {selectedImageInfo !== null && <Archive selectedImageInfo={selectedImageInfo} />}  {/* Active 컴포넌트로 전달 */}
    </div>
  );
};
