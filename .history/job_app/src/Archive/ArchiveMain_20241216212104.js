...
const ArchiveMain = ({setSelectedImageInfo}) => {
  ...
  useEffect(() => {
    ...
    const widthRange = aspectRatio * 50; // 화면의 가로 범위를 확장
    const heightRange = 50; // 화면의 세로 범위를 확장
    ...
    for (let i = 0; i < numImages; i++) {
      ...
      const randomSize = Math.random() * 3 + 1.5; // 이미지 크기 확장
  ...
  images.forEach(({ id, position, size, texture }, index) => {
    const imageTexture = textureLoader.load(texture, (loadedTexture) => {
      ...
      // **렌더 순서 지정**: 큰 인덱스일수록 나중에 렌더링됨(위로 쌓임)
