...
const ArchiveMain = ({ onImageClick }) => {
  ...
  useEffect(() => {
    ...
    const planes = [];
    images.forEach(({ id, name, position, size, texture }) => {
      ...
      plane.userData = { id, name }; // name을 userData에 저장하여 나중에 사용
    ...
    const handleRaycast = () => {
      ...
      if (intersects.length > 0) {
        ...
        intersectedPlane.material.color.set(0x32CD32); // hover된 이미지 초록색으로 변경 (연한 초록)

        // 클릭 이벤트 처리
        const handleClick = (event) => {
          if (intersects.length > 0) {
            const clickedPlane = intersects[0].object;
            const imageName = clickedPlane.userData.name;
            onImageClick(imageName); // 클릭된 이미지 이름을 상위 컴포넌트로 전달
          }
        };

        window.addEventListener("click", handleClick); // 클릭 시 호출
      }
    };
    ...
    return () => {
      ...
      window.removeEventListener("click", handleClick); // 클릭 이벤트 제거
    };
  }, [images, onImageClick]);
