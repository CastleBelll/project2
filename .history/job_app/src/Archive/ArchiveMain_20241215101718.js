...
const ArchiveMain = () => {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (event) => {
      ...
      // 마우스 좌표를 NDC(정규화된 장면 좌표)로 변환
      const mouseX = (event.clientX / window.innerWidth) * 2 - 1;
      const mouseY = -(event.clientY / window.innerHeight) * 2 + 1;
    
      // 카메라 위치를 마우스 이동에 따라 변화시킴
      const zoomFactor = 0.1;
      camera.position.x += (mouseX - camera.position.x) * zoomFactor;
      camera.position.y += (mouseY - camera.position.y) * zoomFactor;
