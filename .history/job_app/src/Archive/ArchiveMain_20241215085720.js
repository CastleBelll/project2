...
const ArchiveMain = () => {
  ...
  const [images, setImages] = useState([]); // 이미지 배열 상태
  ...
  useEffect(() => {
    ...
    for (let i = 1; i <= numImages; i++) {
      ...
      newImages.push(img); // DOM 요소를 newImages 배열에 저장

      container.appendChild(img); // 이미지 요소를 DOM에 추가
    }

    setImages(newImages); // 상태에 저장된 이미지를 새로 고침
    ...
    return () => {
      ...
      // 컴포넌트 언마운트 시 이미지를 제거
      newImages.forEach((img) => container.removeChild(img)); // 새로 생성된 이미지를 제거
    };
  }, []);
      ...
      {images.map((img, index) => (
        <img
          ...
          style={{
            ...
            top: `${img.style.top}`,
            left: `${img.style.left}`,
            width: `${parseInt(img.style.width) * scale}px`,
            height: `${parseInt(img.style.height) * scale}px`,
            transform: `translate(-${(parseInt(img.style.width) * scale) / 2}px, -${(parseInt(img.style.height) * scale) / 2}px)`,
          }}
        />
      ))}
    </div>
