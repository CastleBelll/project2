  ...
  useEffect(() => {
    ...
    const handleWheel = (event) => {
      ...
      images.forEach((img) => {
        ...
        const newSize = currentWidth * zoomFactor;

        // 최소/최대 크기 제한 (50px ~ 300px)
        if (newSize >= 50 && newSize <= 300) {
          img.style.width = `${newSize}px`;
          img.style.height = `${newSize}px`;
          img.style.transform = `translate(-${newSize / 2}px, -${newSize / 2}px)`;
        }
      });
    };
    ...
    return () => {
      // 컴포넌트 언마운트 시 이미지 제거 및 이벤트 리스너 정리
