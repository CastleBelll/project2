...
const ArchiveMain = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      newImages.forEach((img) => container.removeChild(img));
    };
  }, []);
      ...
      {images.map((img, index) => (
        <img
          ...
          style={{
            ...
            top: `${img.y}px`,
            left: `${img.x}px`,
            width: `${img.size * scale}px`,
            height: `${img.size * scale}px`,
            transform: `translate(-${(img.size * scale) / 2}px, -${(img.size * scale) / 2}px)`,
          }}
        />
      ))}
    </div>
