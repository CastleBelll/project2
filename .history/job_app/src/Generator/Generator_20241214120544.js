...
const Generator = () => {
  ...
  const getClickedData = () => {
    ...
    // clickedTexts에 해당하는 텍스트가 포함된 각 카테고리 객체를 묶어서 반환
    const clickedData = data.map((item) => {
      // item.category에서 clickedTexts의 텍스트가 포함되는지 확인
      const matchedItems = Object.entries(item.category)
        .map(([category, subItems]) => {
          const matchedSubItems = subItems.filter(subItem => clickedTexts.includes(subItem));
          return matchedSubItems.length > 0 ? { category, matchedSubItems } : null;
        })
        .filter(Boolean); // null 값은 제거
  
      // 해당 카테고리에서 하나라도 매칭된 텍스트가 있으면 해당 item을 포함
      if (matchedItems.length > 0) {
        return {
          ...item,
          matchedItems, // matchedItems에 텍스트가 포함된 카테고리만 필터링
        };
      }
  ...
  // 원 클릭 시 getClickedData 함수 호출 후 상태에 저장
  const handleCircleClick = () => {
    const newClickedData = getClickedData(); // getClickedData 실행
    setClickedData(newClickedData); // 상태 업데이트
    console.log("Clicked data:", newClickedData); // 확인용 로그
  };
  ...
  return (
    ...
    console.log("Item Image:", item.image);  // 이미지 경로 확인
    console.log("Item Title:", item.title);  // 제목 확인
    console.log("Item Category:", item.category);  // 카테고리 확인
