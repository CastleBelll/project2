...
const Generator = () => {
  ...
  // 우측 화살표 아이콘 클릭 시 배열 초기화 및 원상 복구
  const handleArrowClick = () => {
    ...
    setIsOpen(false); // generator-arial 닫기
  };

  return (
    <div className="generator-container" ref={containerRef}>
      <div className="generator-arial" style={{ display: isOpen ? "block" : "none" }}>
        <button onClick={handleArrowClick}><BsArrowRight size={30} /></button>
      </div>
    </div>
  );
};
