...
const Generator = () => {
  ...
  const getClickedData = () => {
    ...
    // 각 객체마다 clickedTexts의 텍스트가 해당 객체의 category에 포함되는지 확인
    const clickedData = data.filter((item) => {
      // 모든 clickedTexts의 텍스트가 item의 category 내에 포함되어 있는지 확인
      return clickedTexts.every((text) => {
        // category 객체 내의 모든 값들을 평탄화하여 배열로 만든 뒤, 그 값들이 text와 일치하는지 확인
  ...
  return (
    ...
    console.log("Item Category:", item.category);  // 카테고리 확인                return <img key={index} src={`/images/${item.image}`} alt={item.title} />;
              })}
            </div>
