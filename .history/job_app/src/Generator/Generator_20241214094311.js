...
const Generator = () => {
  ...
  const getClickedData = () => {
    ...
    const clickedData = data.filter((item) => {
      const hasAllMatchingSubItems = clickedTexts.every((clickedText) =>
        item.items.includes(clickedText)
      );
      console.log("Checking item:", item);
      console.log("Contains all clicked texts:", hasAllMatchingSubItems);
      return hasAllMatchingSubItems;
    });
  
    console.log("Filtered Clicked Data:", clickedData);
  
    const result = clickedData.map((item) => ({
      image: item.image || "",
      title: item.title || "",
